...
#bsol = blasius(50., 0.1)
u = Float32.(plate_flow(X, Y, U, ν, bsol))
u0 = u[:,start_i,:]
...
model = Chain(encode, x -> n_ode(x), decode)

function predict_n_ode(x)
    model(x)
end
loss_n_ode() = sum(abs,u_train .- reshape(predict_n_ode(u0),numpts,datasize,2))
@load "ed-cpu-checkpoint.bson" w1
@load "du-cpu-checkpoint.bson" w2
...
cb = function ()
    ...
    println(" ")
    flush(stdout)
    @save "ed-cpu-checkpoint.bson" w1
    @save "du-cpu-checkpoint.bson" w2
    ...
    cp = contour(x[start_i:datasize+start_i-1],y,cur_pred[:,:,1],fill=true,
               c=:plasma,clims=(0,U),levels=range(0,stop=U*1.1,length=100))
